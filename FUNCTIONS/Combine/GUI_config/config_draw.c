...
void drawConfigMenu(RESOURCE *res,UIButton *button,DropDownMenu *menu,int unit)
{
    ...
    drawframe(unit);
    drawList(res->fonts[0],&button[RESOLUTION],menu,unit);
    drawCheckBox (res->fonts[0],&button[FULLSCREEN],unit,"Full Screen");
    drawButton (res->fonts[0],&button[PLAY],unit,"Play!");
    drawButton (res->fonts[0],&button[QUIT],unit, "Quit");
